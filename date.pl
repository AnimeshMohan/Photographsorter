...
opendir (Dir, "./$ye/$mo") or die "Could'nt open directory,$!" ;
	...
	{
		if($file eq $day)								#checks if folder for that date exsists
		{
	...
	if($flag == 0)
	{
		mkdir("./$ye/$mo/$day") or die "Could'nt make dir $day,$!" ; 			#creates the folder with that date if it does not exsist
		$d="./$ye/$mo/$day";								
